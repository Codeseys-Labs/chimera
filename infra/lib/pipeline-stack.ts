...
export class PipelineStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: PipelineStackProps) {
    ...
    // The build step resolves Cognito/API/Chat stack outputs to inject VITE_* env
    // vars into the Vite build. Grant CloudFormation read on Security + Chat stacks.
    buildProject.addToRolePolicy(
      new iam.PolicyStatement({
        ...
        resources: [
          ...
          `arn:aws:cloudformation:${this.region}:${this.account}:stack/Chimera-${props.envName}-Chat/*`,
        ],
      })
    );

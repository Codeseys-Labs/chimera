...
tags:
  ...
  - implementation-plan
date: 2026-03-19
topic: ClawCore Self-Evolution Engine
status: complete
---

# ClawCore Self-Evolution Engine

> What makes ClawCore not just an agent platform, but one that **improves itself over time**.
...
Built on: [[ClawCore-Final-Architecture-Plan]]
...
def analyze_conversation_logs(
    ...
    table = dynamodb.Table("clawcore-sessions")
...
prompt_evolution_agent = Agent(
    model=BedrockModel(model_id="us.anthropic.claude-sonnet-4-6-v1:0"),
    system_prompt="""You are the Prompt Evolution Engine for ClawCore.
...
```
Table: clawcore-evolution-state
PK: TENANT#{tenant_id}
...
- This skill was auto-generated by ClawCore's evolution engine
- Generated: {{date}}
- Tenant: {tenant_id}
...
1. **S3 Upload** -- `SKILL.md` and tool code uploaded to `s3://clawcore-skills/{tenant_id}/{skill_name}/`
2. **DynamoDB Registration** -- Metadata written to `clawcore-skills` table
...
permit(
    principal in ClawCore::Role::"agent",
    action == ClawCore::Action::"emergency_heal",
    resource
)
when {
...
def propose_infrastructure_change(
    ...
    auth_result = avp.is_authorized(
        ...
        action={"actionType": "ClawCore::Action", "actionId": "apply_infra_change"},
        resource={
            "entityType": "ClawCore::Infrastructure",
    ...
    codecommit.create_branch(
        ...
        commitId=_get_main_head("clawcore-infra"),
    )
...
def create_self_scheduled_cron(
    ...
    auth = avp.is_authorized(
        policyStoreId="clawcore-policies",
        principal={"entityType": "ClawCore::Agent", "entityId": tenant_id},
        action={"actionType": "ClawCore::Action", "actionId": "create_cron"},
        resource={
            "entityType": "ClawCore::CronJob",
    ...
    scheduler.create_schedule(
        Name=f"clawcore-{tenant_id}-{job_name}",
        ScheduleExpression=f"cron({schedule_expression})",
        FlexibleTimeWindow={"Mode": "OFF"},
        Target={
            "Arn": "arn:aws:states:us-east-1:ACCOUNT:stateMachine:clawcore-cron-executor",
            "RoleArn": "arn:aws:iam::ACCOUNT:role/clawcore-scheduler-role",
            "Input": json.dumps({
                "tenant_id": tenant_id,
                "job_name": job_name,
                "prompt": prompt,
...
permit(
    principal in ClawCore::Role::"agent",
    action == ClawCore::Action::"create_cron",
    resource is ClawCore::CronJob
)
when {
...
permit(
    principal in ClawCore::Role::"agent",
    action in [ClawCore::Action::"modify_cron", ClawCore::Action::"delete_cron"],
    resource is ClawCore::CronJob
)
when {
...
forbid(
    principal in ClawCore::Role::"agent",
    action in ClawCore::ActionGroup::"self_evolution",
    resource
)
when {
...
forbid(
    principal in ClawCore::Role::"agent",
    action == ClawCore::Action::"promote_prompt_variant",
    resource
)
when {
...
forbid(
    principal,
    action == ClawCore::Action::"modify_system_prompt",
    resource
)
when {
...
forbid(
    principal,
    action == ClawCore::Action::"modify_config",
    resource
)
when {
...
forbid(
    principal in ClawCore::Role::"agent",
    action in [
        ClawCore::Action::"create_policy",
        ClawCore::Action::"update_policy",
        ClawCore::Action::"delete_policy"
    ],
    resource is ClawCore::CedarPolicy
);
...
class EvolutionDashboard(cdk.Stack):
    def __init__(self, scope, id, *, tenant_id: str, **kwargs):
        ...
        dashboard = cw.Dashboard(
            ...
            dashboard_name=f"clawcore-evolution-{tenant_id}",
        )
...
### Extended: `clawcore-audit` (existing)
...
Phases reference: [[ClawCore-Final-Architecture-Plan#8. Implementation Phases]]
...
- [[ClawCore-Architecture-Review-Security]] -- STRIDE threat model and Cedar policy design
- [[ClawCore-Architecture-Review-Cost-Scale]] -- Cost model informing model routing optimization
